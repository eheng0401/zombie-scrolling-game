...
public class BasicGame extends AbstractGame {
    ...
    protected void preGame(){
        ...
        score = 0;
        setTitleText("Super Scrolling Game!");	// change title bar
    ...
    protected void spawnEntities()
    {
        ...
        for (int i = 0; i<numSpawn; i++) // spawning entities based on numSpawn
        {
			...
			if (type < 3) // rare get spawn at 2/10 probability
			{
				newSpawn = new RareGet(DEFAULT_WIDTH, rand.nextInt(DEFAULT_HEIGHT-50));
			}
		   
			else if (type < 7)
			{
				newSpawn = new Get(DEFAULT_WIDTH, rand.nextInt(DEFAULT_HEIGHT-50));
			}
			else // if (type >=7)
			{
				newSpawn = new Avoid(DEFAULT_WIDTH, rand.nextInt(DEFAULT_HEIGHT-75));
			}
		   
			while (checkCollision(newSpawn) != null)
			{
				newSpawn.setY(rand.nextInt(DEFAULT_HEIGHT-newSpawn.getHeight()));
			}
    ...
    protected void handleKey(int key){
        ...
        else // (getSplashImage() == null) 
        {
        	...
        	if (!isPaused)
        	{
				...
				{
					if (player.getY()-player.getMovementSpeed() < 0) // ensures player does not move out of frame of gamewindow
						player.setY(0);
					else
